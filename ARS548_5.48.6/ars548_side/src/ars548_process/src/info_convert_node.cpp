...
{
    ros::init(argc, argv, "info_convert_side_node");
    ros::NodeHandle nh;
    ros::NodeHandle private_nh("~");

    ros::Subscriber obj_list_sub = nh.subscribe("/ars548_process_side/object_side_list", 10, &objectReceive);
    ros::Subscriber det_list_sub = nh.subscribe("/ars548_process_side/detection_side_list", 10, &detectionReceive);
    objects_marker_pub = nh.advertise<visualization_msgs::MarkerArray>("/ars548_process_side/object_side_marker", 10);
    point_cloud_pub = nh.advertise<sensor_msgs::PointCloud>("/ars548_process_side/point_side_cloud", 10);
    point_cloud2_pub = nh.advertise<sensor_msgs::PointCloud2>("/ars548_process_side/point_side_cloud2", 10);
