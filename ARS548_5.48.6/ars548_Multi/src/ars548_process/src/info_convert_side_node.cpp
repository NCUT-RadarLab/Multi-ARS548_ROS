...
void objectReceive(const ars548_msg::ObjectList &msg)
{
    ...
    if (size > 0)
    {
        ...
        objects_side_marker_pub.publish(marker_array);
    }
...
void detectionReceive(const ars548_msg::DetectionList &msg)
{
    ...
    // 发布 PointCloud 消息
    point_side_cloud_pub.publish(cloud);

    // 发布 PointCloud2 消息
    point_side_cloud2_pub.publish(cloud_msg2);
}
...
int main(int argc, char **argv)
{
    ...
    ros::Subscriber obj_side_list_sub = nh.subscribe("/ars548_process/object_side_list", 10, &objectReceive);
    ros::Subscriber det_side_list_sub = nh.subscribe("/ars548_process/detection_side_list", 10, &detectionReceive);
