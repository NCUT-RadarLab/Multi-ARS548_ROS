...
void detectionReceive(const ars548_msg::DetectionList &msg)
{
    ...
    // 发布 PointCloud2 消息
    point_cloud2_pub.publish(cloud_msg2);
}

int main(int argc, char **argv)
{
    ros::init(argc, argv, "info_convert_front_node");
    ros::NodeHandle nh;
    ros::NodeHandle private_nh("~");

    ros::Subscriber obj_list_sub = nh.subscribe("/ars548_process_front/object_Front_list", 10, &objectReceive);
    ros::Subscriber det_list_sub = nh.subscribe("/ars548_process_front/detection_Front_list", 10, &detectionReceive);
    objects_marker_pub = nh.advertise<visualization_msgs::MarkerArray>("/ars548_process_front/object_Front_marker", 10);
    point_cloud_pub = nh.advertise<sensor_msgs::PointCloud>("/ars548_process_front/point_Front_cloud", 10);
    point_cloud2_pub = nh.advertise<sensor_msgs::PointCloud2>("/ars548_process_front/point_Front_cloud2", 10);
